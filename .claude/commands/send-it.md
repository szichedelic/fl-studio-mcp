Full lifecycle: branch → organize commits → push → create PR → merge → cleanup.

This is the end-to-end flow from uncommitted changes to merged into main.

## Step 1: Branch

Run `git status` and `git branch --show-current`.

- If no changes and no unpushed commits, abort: "Nothing to send"
...
1. Check PR status: `gh pr view --json state,mergeable,statusCheckRollup`
2. If checks are passing and PR is mergeable:
   - Ask user to confirm merge
   - Merge: `gh pr merge --squash --delete-branch`
3. If checks failing or not mergeable, show status and stop

## Step 5: Cleanup

1. Switch to main: `git checkout main`
2. Pull latest: `git pull origin main`
3. Prune: `git fetch --prune`
