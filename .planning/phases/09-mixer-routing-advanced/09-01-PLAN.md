...
must_haves:
  truths:
    ...
    - "Handler mixer.get_track_sends returns sends for a specific track (by index or name)"
...
**API Limitation Note:** Pre/post fader mode is NOT exposed in FL Studio's Python API. The `setRouteToLevel()` function has no mode parameter. This is documented in 09-CONTEXT.md as a known limitation.
</objective>
...
First, add a helper function for track name resolution (supports BOTH name and index per user decision):
```python
...
2. `handle_mixer_get_track_sends(params)` - Get sends for one track
   - Required: `index` (int) OR `name` (str) - support BOTH per user decision
   - Use `_resolve_track_ref()` to resolve name to index if string provided
   - If resolution fails, return error with helpful message
...
3. `handle_mixer_set_route(params)` - Create or remove a route
   - Required: `source` (int or str), `destination` (int or str), `enabled` (bool)
   - Resolve both source and destination using `_resolve_track_ref()`
   - Validate both resolved indices
   - Call `mixer.setRouteTo(source, destination, 1 if enabled else 0, True)` - True for UI update
   - Readback with `getRouteSendActive()`
   - Return `{success, source, sourceName, destination, destName, active}`

4. `handle_mixer_set_route_level(params)` - Set send level for existing route
   - Required: `source` (int or str), `destination` (int or str), `level` (float 0.0-1.0)
   - Resolve both using `_resolve_track_ref()`
   ...
   - Readback level
   - Return `{success, source, sourceName, destination, destName, level}`
...
IMPORTANT: The full routing table scan (get_routing) iterates trackCount^2 pairs. This is O(n^2) but necessary. FL Studio has ~127 mixer tracks by default, so ~16K checks. Keep it simple, no premature optimization.
  ...
  <verify>Read mixer.py and confirm all 4 handler functions exist with proper validation, `_resolve_track_ref` helper, and register_handler calls.</verify>
  <done>Four routing handlers implemented with name+index support, validation, error handling, and readback patterns matching Phase 8 style.</done>
</task>
...
1. `handle_mixer_get_eq(params)` - Get all EQ band settings for a track
   - Required: `index` (int) OR `name` (str) - use `_resolve_track_ref()`
...
2. `handle_mixer_set_eq_band(params)` - Set EQ band parameters
   - Required: `index` (int or str), `band` (int 0-2)
   - Optional: `gain` (float 0-1), `frequency` (float 0-1), `bandwidth` (float 0-1)
   - Use `_resolve_track_ref()` for index
...
```
  ...
  <verify>Read mixer.py and confirm both EQ handler functions exist with band name mapping, track name resolution, parameter validation, and register_handler calls.</verify>
  <done>Two EQ handlers implemented with name+index support, normalized and real-unit readback, optional parameter updates, and proper band validation.</done>
</task>
...
Run: `diff fl-bridge/handlers/mixer.py "C:/Users/jared/Documents/Image-Line/FL Studio/Settings/Hardware/FLBridge/handlers/mixer.py"` (should show no differences after copy)
  ...
  <done>Module docstring updated with all 12 handlers listed, track reference documentation added, API limitation noted, and runtime copy synced.</done>
</task>
...
3. `_resolve_track_ref` helper exists and is used by routing and EQ handlers
4. Runtime copy matches git repo copy
</verification>
...
- All routing/EQ handlers support BOTH track index and track name
